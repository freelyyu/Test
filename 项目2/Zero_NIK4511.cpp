...
Proof  ZKP(std::vector<NTL::ZZ_p> g, std::vector<NTL::ZZ_p> h,const NTL::ZZ_p w,const NTL::ZZ p,NTL::ZZ q)
{
    ...
    {
        strcpy(digest[i],performHash(2777777*count,2777777*(count+1),g,h,pi.commit));
    //digest[i]=performHash(2777777*count,2777777*(count+1));
    }  
    digest[35]=performHash(2777777*count,g.size(),g,h,pi.commit);
    ...
    for(int i=1;i<36;i++) 
    {
        totalSha.Update(totalDigest, (const unsigned char*)digest[i], CryptoPP::SHA256::DIGESTSIZE);         //.....................
    }
